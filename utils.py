...
def gradient_check_and_clip(loss_op, optimizer,grad_th, ignore_th):
    with tf.name_scope('gradient_clipping'):
        gradients, variables = zip(*optimizer.compute_gradients(loss_op))
        global_norm = tf.global_norm(gradients, name='global_norm')
        gradients = [tf.clip_by_norm(grad, grad_th) for grad in gradients]
        flag_clipped = tf.logical_or(global_norm > ignore_th, tf.logical_not(tf.is_finite(global_norm)))
        gradients = tf.cond(flag_clipped,
                            lambda: [tf.zeros_like(grad) for grad in gradients],
                            lambda: [tf.identity(grad) for grad in gradients])
        return optimizer.apply_gradients(zip(gradients, variables)), flag_clipped
...
def ceil_on_division(a, b):
    return -(-a//b)


global global_variable_list
